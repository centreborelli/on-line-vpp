...
float get_extrapolation(float *mus, float *stds, float mu, int N) {
...
void filter_curve(float *mus_c, float *stds_c, int num_bins,
  ...
  for (int b = 0; b < num_bins; b++) {
    ...
    // Add the extrapolated control points inside the interval [left, right]
    float sum_window = 0;
    int L = 0;
    for (float mu = left; mu <= right; mu += 0.05) {
      sum_window += get_extrapolation(mus_c, stds_c, mu, num_bins);
      L++;
    }
